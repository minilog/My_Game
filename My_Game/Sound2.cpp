...
Sound2* Sound2::instance = nullptr;

Sound2* Sound2::getInstance()
{
	if (instance == nullptr)
		instance = new Sound2(ToanCauGame::get_CuaSoXuLy());

	return instance;
}

Sound2::Sound2(HWND hWnd)
{
	primaryBuffer = 0;
	HRESULT result;
...
void Sound2::create(HWND hWnd)
{
	if (instance == nullptr)
	{
		instance = new Sound2(hWnd);
	}
}

float Sound2::getVolume()
{
	return volume;
}

void Sound2::loadSound(char* fileName, std::string name)
{
...
void Sound2::play(std::string name, bool infiniteLoop, int times)
{
	if (isMute)
	{
		return;
...
void Sound2::stop(std::string name)
{
	if (name == "")
	{
...
void Sound2::setVolume(float percentage, std::string name)
{
	volume = percentage;
	if (name == "")
	{
